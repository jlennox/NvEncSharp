...
public static class LibNvEnc
{
    ...
    public static void Initialize()
    {
        ...
        if (TryInitialize(out var description) != LibNcEncInitializeStatus.Success)
        {
    ...
    public static NvEncoder OpenEncoder(ref NvEncOpenEncodeSessionExParams sessionParams)
    {
        ...
        CheckResult(default, FunctionList.OpenEncodeSessionEx(ref sessionParams, out var encoder));
        ...
        [CallerMemberName] string callerName = "")
    {
        if (status != NvEncStatus.Success)
        {
            throw new LibNvEncException(callerName, encoder.GetLastError(), status);
        }
    }
}